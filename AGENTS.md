...
- **Recruiting presets** – The Recruit All prompt (Vanilla / All Recruitable / No Recruitable) persists through `SetRecruitAllSetting()` (`src/personality_test1.c:429-648`, `src/dungeon_mon_recruit.c:1-110`). Setting it to “All” removes evolution-stage restrictions; “No” hard-disables in-dungeon recruits for pure roguelike runs.
- **Difficulty picker** – Vanilla / Hard / Nightmare choices (`src/data/personality_test1.h:33-56`) write into `SetGameDifficultySetting()` and show up on the load screen (`src/load_screen.c:300-338`). Hooks that scale spawns and boss density can use `GetGameDifficultySetting()`.
