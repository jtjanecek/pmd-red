...
- Implement `GroundScript_ShowTypeSelectionMenu` (exact signature defined in `ground_script.h`) inside a new file such as `src/ground/type_selection_ui.c`. The function should:
  ...
  2. Render a two-option menu using the existing `MenuItem` / `CreateDialogueBoxAndPortrait` helpers so each entry shows the hint text plus the two type icons (we can reuse the standard type-icon drawing functions already used by the summary screen).
