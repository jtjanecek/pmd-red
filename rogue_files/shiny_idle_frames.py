...
def main():
    parser = argparse.ArgumentParser(
        description="Render idle frames (per direction) from AX pose data."
    )
    parser.add_argument(
        "--mon",
        action="append",
    ...
    parser.add_argument(
        "--palette-rows",
        default=None,
        help="Comma-separated palette row list (overrides --palette-count).",
    )
    parser.add_argument(
        "--output-dir",
    ...
    if args.palette_rows:
        palette_rows = parse_palette_rows(args.palette_rows)
    else:
        palette_rows = list(range(args.palette_count))
    ...
    for mon_name, mon_dir in iter_mon_dirs(base_dir, args.mons):
        ...
        try:
            ...
            continue
        if args.anim_id >= len(anims_list):
            print(f"skip {mon_name}: anim_id {args.anim_id} out of range")
            continue
        anim_table_name = anims_list[args.anim_id]
        ...
        for dir_idx, dir_name in enumerate(DIRECTION_NAMES):
            ...
            if args.frame_index >= len(frames):
                print(
                    f"skip {mon_name} {dir_name}: frame_index {args.frame_index} out of range"
                )
                continue
            pose_id, off_x, off_y = frames[args.frame_index]
            ...
            for palette_idx, plte_data in palette_bytes.items():
                out_dir = os.path.join(
                    args.output_dir, mon_name, f"palette_{palette_idx:02d}"
                )
                os.makedirs(out_dir, exist_ok=True)
                out_path = os.path.join(out_dir, f"idle_{dir_name}.png")
                canvas = render_pose(
                    mon_dir, sprite_cache, png_cache, sprite_chunks, entries, (off_x, off_y)
                )
                write_png_indexed(
                    out_path,
                    len(canvas[0]),
