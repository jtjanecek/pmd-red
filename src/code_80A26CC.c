...
// Return whether the dungeon list entry at index is present (selectable)
bool8 sub_80A27CC(s16 index)
{
    s16 entry = 55 + index;
    const DungeonInfo *temp = GetDungeonInfo_80A2608(entry);
    return temp->unk11 != 0;
}

// Map a dungeon list entry index to its dungeonIndex
u8 sub_80A270C(s16 index)
{
    return sub_80A2728(index);
}
