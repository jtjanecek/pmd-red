...
// Out-of-line definition to satisfy references from multiple units.

static inline void BuildBitString(char *out, size_t outSize, s16 varId)
{
...
void DumpStoryFlags(const char *tag)
{
    s32 mainM, mainS;
    ...
    BuildBitString(goOrder, (size_t)sizeof(goOrder), RESCUE_SCENARIO_ORDER_LIST);
    BuildBitString(goJob, (size_t)sizeof(goJob), RESCUE_SCENARIO_JOB_LIST);
    BuildBitString(goConq, (size_t)sizeof(goConq), RESCUE_SCENARIO_CONQUEST_LIST);
    BuildBitString(enterList, (size_t)sizeof(enterList), DUNGEON_ENTER_LIST);
    BuildBitString(clearList, (size_t)sizeof(clearList), DUNGEON_CLEAR_LIST);
    BuildBitString(warpList, (size_t)sizeof(warpList), WARP_LIST);
